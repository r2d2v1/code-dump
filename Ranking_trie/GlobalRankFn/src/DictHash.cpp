				...
				const vector<long> &rankList)
{
	...
	list< list<long> > suffixData;
	list<long> suffixDocID;
	...
	multimap<long,long,greater<long> > rankArray;
	multimap<long,long>::iterator rankIter;

	for(counter=0,recordCounter=0,iterData=data.begin();iterData!=data.end();iterData++){
		if(((string)*iterData).compare("*NEWRECORD")==0){
			//tmpSet.clear();
			//cout<<tmpSet.size()<<" "<<rankArray.size()<<endl;
			for(rankIter=rankArray.begin();rankIter!=rankArray.end();rankIter++){
				tmpSet.push_back(rankIter->second);
			}
			rankArray.clear();
			...
			//cout<<rankArray.insert(rankList[recordCounter])<<endl;
			rankArray.insert(pair<long,long>(rankList[recordCounter],recordCounter));
			//tmpSet.insert(rankArray.insert(rankList[recordCounter]),recordCounter);
			recordCounter++;
	...
	//Trie Flooding
	list< list<long> >::const_iterator pIter;
	list<long>::const_iterator docIDIter;
	list<long> tmp;
	list<long>::const_iterator tIter;
